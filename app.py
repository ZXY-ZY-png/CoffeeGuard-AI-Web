...
def load_models():
    if not DISEASE_MODEL_PATH.exists() or not SEVERITY_MODEL_PATH.exists():
        raise FileNotFoundError("模型文件未找到，请检查 Git LFS 模型文件。")
...
def inspect_zip(uploaded_zip):
    if uploaded_zip is None:return [],None
    try:
        uploaded_zip.seek(0); zf=zipfile.ZipFile(uploaded_zip); infos=[]; total=0
        for info in zf.infolist():
            if info.is_dir() or info.filename.startswith("__MACOSX/"):continue
            if info.flag_bits & 1: zf.close(); return [],"ZIP 中包含加密文件，暂不支持。"
            if Path(info.filename).suffix.lower() in IMAGE_EXTS: infos.append(info); total+=info.file_size
        zf.close()
        if not infos:return [],"ZIP 中没有找到可识别的图片。"
        if len(infos)>MAX_ZIP_IMAGES:return [],f"ZIP 中有 {len(infos)} 张图片，超过单任务上限 {MAX_ZIP_IMAGES} 张。"
        if total>MAX_ZIP_UNCOMPRESSED:return [],"ZIP 解压后的图片总大小超过 1GB，请拆分为多个任务。"
        return infos,None
    except zipfile.BadZipFile:return [],"这不是有效的 ZIP 文件，请不要使用 7z 格式。"
    except Exception as e:return [],f"读取 ZIP 失败：{e}"

init_tasks(); st.markdown('<div class="cg-hero"><h1>☕ CoffeeGuard AI</h1><p>咖啡园病虫害智能巡检与安全 IPM 决策平台 · Web v2.1.1</p></div>',unsafe_allow_html=True)
with st.sidebar:
...
if page=="🏠 平台总览":
    ...
    if f:
        try:
            ...
            with right:
                st.success(f'识别结果：{r["disease"]}'); a,b=st.columns(2); a.metric("AI置信度",f'{r["confidence"]:.2f}%'); b.metric("严重程度",r["severity"]); c,d=st.columns(2); c.metric("综合风险",r["risk"]); d.metric("严重度AI置信度",f'{r["severity_conf"]:.2f}%'); st.bar_chart(pd.Series(r["probs"]))
    ...
    st.subheader("⚡ 快速批量巡检"); fs=st.file_uploader("选择多张咖啡叶图片",type=["jpg","jpeg","png","bmp","webp"],accept_multiple_files=True,key="batch")
    if fs and st.button("开始批量识别",type="primary"):
        rows=[]; bar=st.progress(0)
        for i,f in enumerate(fs,1):
            try:r=predict(Image.open(f).convert("RGB")); save_history(f.name,r); rows.append({"图片":f.name,"识别结果":r["disease"],"AI置信度(%)":round(r["confidence"],2),"严重程度":r["severity"],"综合风险":r["risk"]})
            except Exception as e:rows.append({"图片":f.name,"识别结果":"失败","错误":str(e)})
            bar.progress(i/len(fs))
        rdf=pd.DataFrame(rows); st.dataframe(rdf,use_container_width=True,hide_index=True); st.download_button("⬇️ 下载本次巡检 CSV",rdf.to_csv(index=False).encode("utf-8-sig"),"CoffeeGuard_batch_results.csv","text/csv")
elif page=="🗂️ 正式巡检任务":
    st.subheader("🗂️ 正式巡检任务中心"); st.caption("支持两种导入方式：直接多选图片，或上传整个图片文件夹压缩成的 ZIP。")
    with st.expander("➕ 创建新任务",expanded=not st.session_state.formal_tasks):
        name=st.text_input("任务名称",value=f"咖啡园巡检_{datetime.now().strftime('%Y%m%d_%H%M')}"); note=st.text_input("任务备注（可选）")
    ...
    if not st.session_state.formal_tasks:st.info("暂无正式巡检任务，请先创建任务。")
        ...
        labels=[f'{t["name"]} · {t.get("status","待巡检")} · {t["created"]}' for t in st.session_state.formal_tasks]; idx=st.selectbox("选择任务",range(len(labels)),format_func=lambda i:labels[i]); t=st.session_state.formal_tasks[idx]; ok,high,major=task_summary(t); a,b,c,d=st.columns(4); a.metric("任务图片",t.get("total",0)); b.metric("成功识别",ok); c.metric("高/较高风险",high); d.metric("主要病害",major)
        st.markdown("#### 方式 A：直接选择多张图片"); fs=st.file_uploader("选择多张咖啡叶图片",type=["jpg","jpeg","png","bmp","webp"],accept_multiple_files=True,key=f'taskfiles_{t["id"]}')
        st.markdown("#### 方式 B：上传 ZIP 图片文件夹"); zup=st.file_uploader("上传 ZIP 压缩包（不支持 .7z）",type=["zip"],key=f'taskzip_{t["id"]}')
        zinfos,zerr=inspect_zip(zup)
        if zup:
            if zerr:st.error(zerr)
            else:
                size_mb=sum(x.file_size for x in zinfos)/1024/1024; st.success(f"ZIP 检查完成：发现 {len(zinfos)} 张图片，解压后约 {size_mb:.1f} MB。")
                if len(zinfos)>500:st.warning("图片较多，云端 CPU 处理会比较慢。请保持页面开启，必要时拆分为多个任务。")
        source_count=len(fs)+(len(zinfos) if not zerr else 0)
        if source_count:st.info(f"本次准备巡检：{source_count} 张图片。")
        if source_count and st.button("▶ 开始正式任务巡检",type="primary"):
            t["total"]=source_count; t["rows"]=[]; t["status"]="巡检中"; bar=st.progress(0); status=st.empty(); done=0
            def add_result(name,img):
                try:
                    r=predict(img); save_history(name,r); t["rows"].append({"图片":name,"识别结果":r["disease"],"AI置信度(%)":round(r["confidence"],2),"严重程度":r["severity"],"严重度AI置信度(%)":round(r["severity_conf"],2),"综合风险":r["risk"]})
                except Exception as e:t["rows"].append({"图片":name,"识别结果":"失败","AI置信度(%)":"","严重程度":"","严重度AI置信度(%)":"","综合风险":"","错误":str(e)})
            for f in fs:
                done+=1; status.write(f"正式任务识别 {done}/{source_count}：{f.name}"); add_result(f.name,Image.open(f).convert("RGB")); bar.progress(done/source_count)
            if zup and zinfos and not zerr:
                zup.seek(0)
                with zipfile.ZipFile(zup) as zf:
                    for info in zinfos:
                        done+=1; status.write(f"ZIP巡检 {done}/{source_count}：{info.filename}")
                        try:
                            with zf.open(info) as raw:add_result(info.filename,Image.open(raw).convert("RGB"))
                        except Exception as e:t["rows"].append({"图片":info.filename,"识别结果":"失败","错误":str(e)})
                        bar.progress(done/source_count)
            t["status"]="已完成"; status.success(f"正式巡检完成：共处理 {source_count} 张图片。"); st.rerun()
        if t.get("rows"):
    ...
    st.subheader("🛡️ 安全 IPM 决策中心"); st.markdown("""<div class="safe"><b>系统原则</b><br>AI负责辅助识别、严重度评估和风险分级，不自动替代现场诊断，也不自动“开农药”。优先采用现场复核、田间卫生、通风透光、树势管理、生物防治与持续监测。</div>""",unsafe_allow_html=True)
    for k in CLASSES:
...
st.divider(); st.caption("CoffeeGuard AI Web v2.1.1 · ZIP文件夹批量巡检 · AI置信度不等同于真实Accuracy")